...
import { IonContent, IonButton, IonInput, IonCard, IonCardHeader, IonCardTitle, IonCardSubtitle, IonText, IonIcon, IonSpinner } from '@ionic/angular/standalone';
...
@Component({
  ...
  standalone: true,
  imports: [IonSpinner, IonIcon, IonText, IonCardSubtitle, IonCardTitle, IonCardHeader, IonCard, IonInput, IonButton, IonContent, CommonModule, FormsModule,RouterLink, Header2Page]
})
...
export class WeatherPage implements OnInit {
 ...
 // Initilises several services and routers to be used by the class
  ...
  ngOnInit() {
    // This retrieves the apikey from the Apiservice
  ...
  ionViewWillEnter(){
   ...
   async getCapitalWeather() {
    ...
    try {
      ...
      //Checks if there is a 404 code sent back in the data
      
      if(this.capitalWeather.cod === '404'){
        this.isWeather = false;
        this.message = `No weather data for  ${this.country}`
      }
...
// This sets the error flag to true if the location doesnt exist in the country the user is searching in.
  ...
  try {
   // Uses helper method to get data
    const response = await this.getWeatherDataFromService(url);
    let location = response.data
    //  // If the name in the data doesnt match the user input it is set to the user input
    //  // This is to avoid situation where different APIs can return different 
    ...
    //   }
     ...
     //Pushes the weather item to the weatherLocations array 
     this.weatherLocations.push(location)
    // This passed the weather item parameters to the method to save the items in storage.
  ...
  //This method loads all the weather locations saved previously by the user from storage
  async getFavouriteWeatherLocations(){
    ...
    this.storedWeatherLocations= response;
     ...
     // stored locations can be retrieved from the API
...
async getStoredLocationsCurrentWeather(id:  number, name : string){
  ...
  // This url allows for the unque id to be passed from the items retrieved from the storage array to the api directly 
...
//This method saves the weather location added by the user automatically to storage
  ...
  saveWeatherLocations(id : number, name : string){
    ...
    // Used the data service to save the items to the storage array
    this.mds.saveItemToArray("weather" + this.countryCode, location )
    
  }

  //Removes the location from the array using the key of the unique id number of the 
  async removeLocation(id : number){
    ...
    try {
      ...
      // Fetch the updated weather data by calling the ionic life cycle again.
